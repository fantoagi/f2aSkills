...
- 若输入是 markdown 文件，发布前先做一次轻量规范化：剥离 YAML frontmatter（如 `title:`、`cover:`、`tags:`、`date:` 等元数据段），只把正文部分送进飞书；frontmatter 只能用于本地元信息，不可作为飞书正文
- frontmatter 判定规则：仅当文件开头命中 `---` 起始块，且在后续再次命中配对的 `---` 结束块时，才把这整段视为 frontmatter 去掉；不要删除正文中间的普通分隔线
- 若规范化后第一屏仍出现类似 `title:` / `cover:` / `tags:` / `date:` 这类成组元数据键值，先停止发布，说明清洗失败，不要把这类内容直接发到飞书
- 更新已有文档时，若本次新增文章已有独立标题，正文块中不要再把 frontmatter 里的 `title` / `cover` 文本重复带入开头
- 若本次是“给已有栏目追加一篇新文章 / 新章节”，必须先从当前 fetch 结果里定位该栏目的**结束锚点**与**下一栏目标题**；默认把新增内容插在“目标栏目最后一篇现有文章之后、下一栏目标题之前”，不要把整篇文档末尾当成安全 append 点
- 若任务是“给全文最后一个栏目继续追加”，也必须先验证“文档末尾 = 目标栏目末尾”后才能用 `append`；否则一律改用 `insert_before` / `insert_after`
- 若无法唯一定位“目标栏目末尾 / 下一栏目标题”边界，不要继续发布，先缩小范围、补充锚点或切换到更明确的 insert 方案
- 正文中凡是涉及架构、流程、关系、时间线、对比、因果等结构内容，默认要在对应段落后插入 `<whiteboard type="blank"></whiteboard>`；如果草稿里已经出现流程式代码块、ASCII 箭头步骤、Mermaid/PlantUML/DSL 片段，不要原样保留为纯代码块，必须转成合法 whiteboard 路线
...
- 是否存在必须保留且不可覆盖的对象区域
- 如果是向已有栏目追加新文章：新增内容的插入点是不是被唯一定位为“目标栏目内部”，而不是整篇文档末尾
- 当前线上是否已存在同标题文章、重复标题或疑似残留重复块；若已存在，先判断是补写、移动还是修复，禁止直接再次插入同名整块内容
...
- 追加到已有栏目时，优先 `insert_before` 下一栏目标题、或 `insert_after` 目标栏目最后一个已确认对象；只有当“文档末尾 = 目标栏目末尾”被明确验证时才允许直接 `append`
- 发布前先做一次重复保护检查：同标题是否已存在、目标范围是否已有上次失败留下的重复块；未判清前不要直接插入
...
6. 检查本次新增文章标题在全文中的出现次数是否符合预期；若是修复重复块，必须显式验证重复已消失
7. 检查草稿中的流程式代码块 / Mermaid / ASCII 流程片段没有以纯代码块形式残留在线上正文里
...
- 旧本地稿覆盖了用户后来的线上手动修改
- 新增文章被插到了整篇文档末尾，而不是目标栏目内部
- 流程式代码块被原样发上去，没有转成 whiteboard
- 修复时先插入新块再删旧块，导致同标题文章短暂或长期重复
