...
- 若输入是 markdown 文件，发布前先剥离文件开头成对 `---` 包裹的 YAML frontmatter；其中 `title`、`cover`、`tags`、`date` 等字段只作为本地元信息，不进入飞书正文
- frontmatter 清洗后，检查正文开头是否仍残留成组元数据键值；若仍像 `title: ...`、`cover: ...` 这种连续键值串，说明清洗失败，应先止损，不要继续发布
...
- 若任务看起来像“追加到最后一个栏目”，也要先验证“文档末尾 = 目标栏目末尾”；只有验证成立时，`append` 才是安全路线
